        ...
        $("body").on('click', '.card-product-swatch .option-swatch label',function(){
            ...
            .then((res) => {
                ...
                if(objs != null){
                    ...
                    const availableToSell = selectedOptions.inventory.aggregated.availableToSell;
                    ...
                    if(basePrice != salePrice)
                        $(this).closest("article").find(".wb-sale-text").html(Math.ceil(((basePrice-salePrice)*100)/basePrice))

                    if(availableToSell == 0)
                        $(this).closest("article").find(".productBadges .sold_out").addClass("hidden");
                    else
                        $(this).closest("article").find(".productBadges .sold_out").removeClass("hidden");
